...
**資料來源**：
- 融資／融券的餘額、增減、使用率：透過 Playwright 攔截 CMoney 融資融券頁面的官方計算 API（`GetMarketMarginTradingInfo`）。
- 融資維持率：自行計算（`maintenance_calc.py`），資料來自 TWSE 開放資料 API（個股融資餘額、個股收盤價）與 TWSE 大盤融資金額彙總。
...
- **臺灣證券交易所 (TWSE) API、網路爬蟲**：券資比、加權股價指數、融資維持率（自行計算所需的個股融資餘額、收盤價、大盤融資金額）
- **臺灣期貨交易所 (TAIFEX) 網路爬蟲**：VIX 指數、期貨未平倉口數
- **CMoney（透過 Playwright 攔截官方計算 API）**：大盤融資融券（餘額/增減/使用率）、台指期夜盤指數
...
│   ├── maintenance_calc.py       # 融資維持率自行計算（不含 ETF，比照財經M平方）
...
- 大盤融資融券的餘額／增減／使用率採 CMoney 計算結果；融資維持率（v2.2 起）為自行計算，不含 ETF
